...
function StatBlock({ value, suffix, label }: { value: number; suffix: string; label: string }) {
  ...
  return (
    <div ref={ref} className="bg-navy-900 border border-navy-800 p-6 text-center">
      <div className="text-4xl font-bold text-white tabular-nums">{count}{suffix}</div>
      <p className="mt-2 text-xs text-navy-500 uppercase tracking-wider">{label}</p>
    </div>
  );
}
    ...
    <section ref={sectionRef} className="py-24 md:py-32 bg-navy-950 relative overflow-hidden">
      ...
      <div className="container relative">
        <div className="max-w-3xl mx-auto text-center mb-16">
          <div
            className={`inline-flex items-center gap-2 px-4 py-2 bg-warning-500/10 border border-warning-500/20 mb-8 opacity-0 ${isVisible ? 'animate-fade-up' : ''}`}
          ...
          <div className="grid md:grid-cols-3 gap-px bg-navy-700">
            {timeline.map((item) => {
              ...
              return (
                <div
                  key={item.year}
                  className={`bg-navy-900 p-6 text-center relative ${isCurrent ? 'ring-2 ring-warning-500 ring-inset' : ''}`}
                >
                  ...
                  <div className={`w-12 h-12 mx-auto flex items-center justify-center mb-4 ${
                    isCurrent ? 'bg-warning-500/20 text-warning-400' : isFuture ? 'bg-danger-500/20 text-danger-400' : 'bg-navy-800 text-navy-400'
                  }`}>
                    <item.icon className="w-6 h-6" />
                  </div>
                  
                  <p className={`text-2xl font-bold ${isCurrent ? 'text-warning-400' : isFuture ? 'text-danger-400' : 'text-navy-500'}`}>
                    {item.year}
                  </p>
        ...
        <div
          className={`grid grid-cols-3 gap-px bg-navy-700 max-w-2xl mx-auto mb-16 opacity-0 ${isVisible ? 'animate-fade-up' : ''}`}
        ...
        >
          <div className="bg-navy-900 border border-warning-500/30 p-8 text-center">
          ...
          <div className="grid md:grid-cols-2 gap-px bg-navy-700">
            {factors.map((factor, index) => {
              ...
              return (
                <div
                  key={index}
                  className="bg-navy-900 p-5 flex gap-4"
                >
                  <div className={`w-10 h-10 bg-navy-800 flex items-center justify-center shrink-0 ${colorStyles[factor.color as keyof typeof colorStyles]}`}>
        ...
        >
          <div className="bg-navy-900 border-l-4 border-warning-500 p-6 flex items-center gap-6">
